...
public class ReceptionMainDashController implements Initializable {

    @FXML
    private AnchorPane receptionMainDashAnchPane;
    @FXML
    ...
    @FXML
    private ImageView reserveAppointmentIcon;

    @Override
    public void initialize(URL location, ResourceBundle resources) {
        System.gc();

        selectionTitleLbl.setText("Welcome to Reception's Main Dash.");
    ...
    private void loadLogoutHBoxAction() {
        ...
        if (action.get() == ButtonType.OK) {
            try {
                ...
                Scene temp = new Scene(root);
                Stage stage = (Stage) this.receptionMainDashAnchPane.getScene().getWindow();
                stage.setScene(temp);
                stage.centerOnScreen();
    ...
    @FXML
    public void receptionMainDashIcon_onMouseEntered(MouseEvent mouseEvent) {
        if (mouseEvent.getSource() instanceof ImageView) {
            ImageView icon = (ImageView) mouseEvent.getSource();

            switch (icon.getId()) {
                case "patientIcon":
                    selectionTitleLbl.setText("Patient");
                    selectionDescriptionLbl.setText("Click to view, add, update, delete patients.");
                    break;
                case "reserveAppointmentIcon":
                    selectionTitleLbl.setText("Appointment");
                    selectionDescriptionLbl.setText("Click to make appointments with doctors.");
                    break;
            }
    ...
    @FXML
    public void receptionMainDashIcon_onMouseExited(MouseEvent mouseEvent) {
        ImageView icon = (ImageView) mouseEvent.getSource();
        ScaleTransition scaleT = new ScaleTransition(Duration.millis(200), icon);
        scaleT.setToX(1);
        scaleT.setToY(1);
    ...
    @FXML
    public void patientIcon_onMouseClicked(MouseEvent event) {
        loadReceptionDashBoardOptions("recep_patientManagement", "Patients");
    }

    @FXML
    public void reserveAppointmentIcon_onMouseClicked(MouseEvent event) {
        loadReceptionDashBoardOptions("recep_appointmentManagement", "Appointments");
    }

    public void loadReceptionDashBoardOptions(String fxmlFileName, String title) {
        try {
            Parent root = FXMLLoader.load(this.getClass().getResource("/lk/ijse/dcs/view/" + fxmlFileName + ".fxml"));
            Scene temp = new Scene(root);
            Stage stage = (Stage) this.receptionMainDashAnchPane.getScene().getWindow();
            stage.setScene(temp);
            stage.setTitle("New Philip Hospitals - Doctor Channelling System >>> Reception's Main DashBoard >>>" + title);
            stage.setResizable(false);
            stage.show();
