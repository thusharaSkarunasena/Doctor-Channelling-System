...
public class PharmacistMainDashController implements Initializable {

    @FXML
    private AnchorPane pharmacistMainDashAnchPane;
    @FXML
    ...
    @FXML
    private ImageView issueDrugsIcon;
    @FXML
    private ImageView manageDrugIcon;
    @FXML
    private ImageView previousDrugIssueIcon;

    @Override
    public void initialize(URL location, ResourceBundle resources) {
        System.gc();

        selectionTitleLbl.setText("Welcome to Pharmacist's Main Dash.");
    ...
    private void loadLogoutHBoxAction() {
        ...
        if (action.get() == ButtonType.OK) {
            try {
                ...
                Scene temp = new Scene(root);
                Stage stage = (Stage) this.pharmacistMainDashAnchPane.getScene().getWindow();
                stage.setScene(temp);
                stage.centerOnScreen();
    ...
    @FXML
    void pharmacistMainDashIcon_onMouseEntered(MouseEvent mouseEvent) {
        if (mouseEvent.getSource() instanceof ImageView) {
            ImageView icon = (ImageView) mouseEvent.getSource();

            switch (icon.getId()) {
                case "issueDrugsIcon":
                    selectionTitleLbl.setText("Issue Drugs");
                    selectionDescriptionLbl.setText("Click to view & issue drugs to patients.");
                    break;
                case "manageDrugIcon":
                    selectionTitleLbl.setText("Manage Drugs");
                    selectionDescriptionLbl.setText("Click to Add , Update, Delete and View All Drugs.");
                    break;
                case "previousDrugIssueIcon":
                    selectionTitleLbl.setText("Previous Drug Send Outs");
                    selectionDescriptionLbl.setText("Click to see previous works.");
                    break;
            }
    ...
    @FXML
    void pharmacistMainDashIcon_onMouseExited(MouseEvent mouseEvent) {
        ImageView icon = (ImageView) mouseEvent.getSource();
        ScaleTransition scaleT = new ScaleTransition(Duration.millis(200), icon);
        scaleT.setToX(1);
        scaleT.setToY(1);
    ...
    @FXML
    void issueDrugsIcon_onMouseClicked(MouseEvent event) {
        loadPharmacistDashBoardOptions("pharm_issueDrugs","Issue Drugs");
    }

    @FXML
    void manageDrugIcon_onMouseClicked(MouseEvent event) {
        loadPharmacistDashBoardOptions("pharm_drugManagement","Manage Drugs");
    }

    @FXML
    void previousDrugIssuingsIcon_onMouseClicked(MouseEvent event) {
        loadPharmacistDashBoardOptions("pharm_previousSendOuts","Previous Drug Send Outs");
    }

    public void loadPharmacistDashBoardOptions(String fxmlFileName, String title) {
        try {
            Parent root = FXMLLoader.load(this.getClass().getResource("/lk/ijse/dcs/view/" + fxmlFileName + ".fxml"));
            Scene temp = new Scene(root);
            Stage stage = (Stage) this.pharmacistMainDashAnchPane.getScene().getWindow();
            stage.setScene(temp);
            stage.setTitle("New Philip Hospitals - Doctor Channelling System >>> Pharmacist's Main DashBoard >>>" + title);
            stage.setResizable(false);
            stage.show();
